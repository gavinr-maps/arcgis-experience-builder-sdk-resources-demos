  ...
  onExecute (message: Message, actionConfig?: any): Promise<boolean> | boolean {
    ...
    switch (message.type) {
      case MessageType.StringSelectionChange:
        q = `OBJECTID = '${(message as StringSelectionChangeMessage).str}'`
        break
      case MessageType.DataRecordsSelectionChange:
        q = 'OBJECTID = ' +
          `${(message as DataRecordsSelectionChangeMessage).records.length > 0 ? `'${(message as DataRecordsSelectionChangeMessage).records[0].getFieldValue('OBJECTID')}'` : ''}`
        break
    }
